...
//  WiFi.begin("LarroBrun","27067243LB");
  Debug.println("Attempting to connect to SSID " + configuration->Wifi->ssid + "... password=" + configuration->Wifi->password + " " + String(configuration->Wifi->password.length()));
...
void WifiManager::configurationChanged(){
  ...
  if ((configuration->Wifi->ssid != lastSSID) || (configuration->Wifi->password != lastPassword)){
    Debug.println("WifiManager::configurationChanged()");
    setup();
  }
}
